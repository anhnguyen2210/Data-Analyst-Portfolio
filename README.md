# Data-Analyst-Portfolio
# 👋 Nguyen Le Anh Nguyen (Wynn) — Data Analyst | Product & Operations Analytics

## 📌 About Me

I am a Data Analyst with 4+ years of experience working at the intersection of **product, operations, and business analytics**.

My core strength lies in transforming complex data into **actionable insights that drive real business impact** — from revenue optimization and user behavior analysis to large-scale operational performance monitoring.

I have worked across:
- E-commerce (Monetization & Growth)
- Logistics (Operations & Network Optimization)
- Product Analytics (A/B Testing, Funnel Optimization)
---

## 🎯 Key Achievements

- 🚀 Increased conversion rate by **+30%** through A/B testing optimization  
- 💰 Drove **10x revenue growth** via bundle personalization strategy  
- 📦 Improved logistics productivity by **+35–40%** through operational analytics  
- 📊 Built centralized dashboards & automated pipelines, **cutting reporting workload by 50%**

---

## 🛠️ Technical Skills

**Data Analysis**
- SQL (BigQuery, Trino)
- Python (Pandas, NumPy)

**Visualization**
- Power BI  (DAX)
- Tableau  
- Looker Studio
- Superset
- Metabase

**Analytics**
- A/B Testing  
- Funnel Analysis  
- Cohort Analysis  
- KPI Design  

**Data**
- Data Modeling  
- ETL  
- Dashboard Development  
---

# 📊 Portfolio Projects

This section showcases selected projects demonstrating my ability to solve real-world business problems using data.

---

## 🔥 1. Revenue Performance Analysis (Power BI) | [LINK]([url](https://github.com/anhnguyen2210/Revenue-Performance-Analysis-Dashboard-Olist-E-commerce)))

### 📌 Overview
End-to-end revenue analysis for an e-commerce platform using Olist dataset.

### 🎯 Business Problem
- Identify key revenue drivers  
- Diagnose performance issues  
- Optimize growth strategy  

### 📊 Key Highlights
- Analyzed ~100K orders across multi-dimensional dataset  
- Identified **revenue concentration in top regions & categories**  
- Detected **operational inefficiencies impacting performance**  

### 💡 Key Insights
- Revenue heavily concentrated in a few regions (SP dominates)
- Top categories drive majority of revenue
- Late delivery (~9%) impacts customer experience
- Customer purchasing behavior is weekday-driven

### 💰 Business Impact
- Proposed regional expansion strategy  
- Identified logistics bottlenecks  
- Improved decision-making for revenue optimization  

### 🛠️ Tools
- Power BI  
- SQL  

---

## 🔥 2. Customer Retention & LTV Analysis (Olist Dataset) | [LINK]([url](https://github.com/anhnguyen2210/brazilian_ecommerce_retention_ltv_olist))

### 📌 Overview
Customer behavior analysis focusing on retention and lifetime value.

### 🎯 Business Problem
- Understand churn behavior  
- Identify high-value customers  
- Improve retention strategy  

### 📊 Key Highlights
- Built cohort retention model across customer lifecycle  
- Analyzed retention drop-off patterns  
- Estimated LTV distribution  

### 💡 Key Insights
- Majority of users churn within first 3 months  
- Small group of users drives majority of revenue (Pareto effect)  

### 💰 Business Impact
- Suggested early-stage retention strategies  
- Identified high-value customer segments for targeting  

### 🛠️ Tools
- SQL  
- Python  
- Power BI  

---

# 📈 What Makes My Work Different

Most data portfolios focus on:
❌ dashboards  
❌ tools  

My work focuses on:
✅ business problems  
✅ decision-making  
✅ measurable impact  

---

# 🚀 Current Focus

I am currently focused on:
- Product Analytics (Growth, Monetization)
- Experimentation (A/B Testing)
- Advanced Analytics (LTV, Retention)
- Bridging Data & Business Strategy  

---

# 📬 Contact

- 📧 Email: anhnguyen22102000@gmail.com  
- 🔗 LinkedIn: https://www.linkedin.com/in/anhnguyen221/
- 💻 GitHub: [https://github.com/your-github  ](https://github.com/anhnguyen2210)

---

# ⭐ Final Thought

> "The value of data is not in dashboards — it's in decisions."
